...
var HardwareSimulator = function() {
  ...
  this.run = function(testFilePath) {
    ...
    var testScriptFile = fs.readFileSync(testFilePath);
    var testScript = stripCommentsAndBlankLines(testScriptFile.toString());
  ...
  var simulate = function(simulationSteps, chip, config) {
    ...
    simulationSteps.forEach(function(step) {
      step.forEach(function(command) {
        switch(command.command) {
          case "set":
            chip.setValue(command.variable, command.value);
            break;
          case "eval":
            // no op
            break;
          case "output":
            var columns = config.variables.map(function(v) {
              ...
              var format = "%0" + v.width + v.format.toLowerCase();
              return leftPadding + sprintf(format, value) + rightPadding;
            });
