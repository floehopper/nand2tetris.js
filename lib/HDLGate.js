...
var HDLGate = Gate.extend({
  initialize: function(chipClass) {
    ...
    var tree = parser.parse(hdl.toString(), { AST: AST });
    var parts = tree.parts;

    Gate.prototype.initialize.apply(this, [tree.inputs.pins, tree.outputs.pins]);

    if (tree.builtin) {
      try {
        chipClass = require("../lib/" + tree.builtin.name);
      } catch(exception) {
      ...
      tree.inputs.pins.forEach(function(input) {
        if (chip.hasInput(input) && gate.hasInput(input)) {
          gate.getInput(input).on("change:value", function() {
            chip.setInputValue(input, gate.getInputValue(input));
          });
        } else {
          throw "'" + input + "' is not an input pin in " + tree.builtin.name;
        };
      });
      tree.outputs.pins.forEach(function(output) {
        if (chip.hasOutput(output) && gate.hasOutput(output)) {
          chip.getOutput(output).on("change:value", function() {
            gate.setOutputValue(output, chip.getOutputValue(output));
          });
        } else {
          throw "'" + output + "' is not an output pin in " + tree.builtin.name;
        };
      });
    } else {
