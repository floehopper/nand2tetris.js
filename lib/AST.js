...
AST.Busses.prototype.toDefinition = function() {
  var busses = this.busses.map(function(bus) { return bus.toDefinition(); });
  return new Definition.Busses({ busses: busses });
};
...
AST.Bus.prototype.toDefinition = function() {
  return new Definition.Bus({ name: this.name, width: this.width });
};
