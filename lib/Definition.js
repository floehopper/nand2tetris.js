...
Definition.Chip.prototype.buildModel = function() {
  var gate = new Model.Chip({
    name: this.name,
    inputs: this.inputs.buildModel(),
    outputs: this.outputs.buildModel(),
...
Definition.Connection.prototype.resolve = function(chip) {
  if ((chip.inputs.pins + chip.outputs.pins).indexOf(this.chip_pin) == -1) {
    chip.internals.pins.push(this.chip_pin);
  };
};
