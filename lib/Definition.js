      ...
      part.connections.forEach(function(connection) {
        if (chip.hasInput(connection.part_pin)) {
          if (gate.hasInput(connection.chip_pin)) {
            var input = gate.getInput(connection.chip_pin);
            input.on("change:value", function() {
              chip.setInputValue(connection.part_pin, input.getValue());
            });
          } else {
            var internal = gate.getInternal(connection.chip_pin);
            internal.on("change:value", function() {
              chip.setInputValue(connection.part_pin, internal.getValue());
            });
          };
        } else if (chip.hasOutput(connection.part_pin)) {
          if (gate.hasOutput(connection.chip_pin)) {
            var output = chip.getOutput(connection.part_pin);
            output.on("change:value", function() {
              gate.setOutputValue(connection.chip_pin, output.getValue());
            });
          } else {
            var output = chip.getOutput(connection.part_pin);
            output.on("change:value", function() {
              gate.setInternalValue(connection.chip_pin, output.getValue());
            });
          };
        } else {
