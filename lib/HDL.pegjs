...
chip =
  "CHIP" required_whitespace name:chip_name begin_chip statements:statement* end_chip
  { return new AST.Chip({ name: name, statements: statements }); }
...
statement=
  inputs / outputs / builtin / parts

builtin =
  "BUILTIN" required_whitespace name:chip_name optional_whitespace terminator
...
inputs =
  ...
  { return new AST.Inputs({ inputs: pin_list ? pin_list : [] }); }

outputs =
  "OUT" pin_list:pin_list? terminator
  { return new AST.Outputs({ outputs: pin_list ? pin_list : [] }); }
