...
chip =
  optional_whitespace "CHIP" required_whitespace name:chip_name begin_chip impl:chip_implementation end_chip
  { return new AST.Chip({ name: name, inputs: impl.inputs, outputs: impl.outputs, parts: impl.parts, builtin: impl.builtin }); }
...
builtin =
  ...
  { return new AST.Builtin({ name: name }); }

parts =
  "PARTS:" part_list:part_list?
  { return new AST.Parts({ parts: part_list ? part_list : [] }); }
...
part =
  ...
  { return new AST.Part({ name: part_name, connections: connections }); }
...
connection =
  ...
  { return new AST.Connection({ part_pin: part_pin, chip_pin: chip_pin }); }
...
inputs =
  ...
  { return new AST.Busses({ busses: bus_list ? bus_list : [] }); }
...
bus_name =
  ...
  { return bus_width ? new AST.Bus({ name: identifier, width: bus_width }) : new AST.Bus({ name: identifier }); }
